...
function safeAmount(value) {
  ...
  const isSafeExpression = new RegExp("^[0-9+*/().-]+$").test(normalized);
...
function App() {
  ...
  const filteredExpenses = useMemo(() => {
    if (expenseFilter === "AA") return sortedExpenses.filter((expense) => expense.isAA);
    if (expenseFilter === "不AA") return sortedExpenses.filter((expense) => !expense.isAA);
    return sortedExpenses;
  }, [sortedExpenses, expenseFilter]);
  ...
  const stats = useMemo(() => {
    ...
    const paidAll = Object.fromEntries(people.map((p) => [p, 0]));
    const shareAll = Object.fromEntries(people.map((p) => [p, 0]));

    for (const e of data.expenses) {
      const amount = safeAmount(e.amount);
      if (paidAll[e.payer] !== undefined) paidAll[e.payer] += amount;
      for (const p of people) {
        shareAll[p] += shareFor(e, p);
        shareAA[p] += shareFor(e, p, true);
      }
      if (e.isAA && paidAA[e.payer] !== undefined) paidAA[e.payer] += amount;
    }

    const nets = people.map((p) => ({ name: p, net: (paidAA[p] || 0) - (shareAA[p] || 0) }));
    ...
    for (const e of data.expenses) {
      const amount = safeAmount(e.amount);
      const day = dailyMap.get(e.date) || { date: e.date, total: 0, myShare: 0 };
      day.total += amount;
      day.myShare += shareFor(e, data.me);
      dailyMap.set(e.date, day);

      const cat = categoryMap.get(e.category) || { category: e.category, total: 0, myShare: 0 };
      cat.total += amount;
      cat.myShare += shareFor(e, data.me);
  ...
  }, [data.expenses, data.me, people.join("|")]);
  ...
  function applyPeople() {
    ...
    setData((prev) => ({
      ...prev,
      people: nextPeople,
      me: nextPeople.includes(prev.me) ? prev.me : nextPeople[0],
    }));
    setExpenseForm((prev) => ({
      ...prev,
      payer: nextPeople.includes(prev.payer) ? prev.payer : nextPeople[0],
  ...
  function addExpense(e) {
    ...
    const participants = expenseForm.isAA
      ? expenseForm.participants.filter((p) => people.includes(p))
      : [];
    ...
    setData((prev) => ({
      ...
      expenses: [
        ...prev.expenses,
        {
          ...
          amount,
          participants: expenseForm.isAA ? participants : [],
        },
      ],
    }));
  ...
  function addLog(e) {
    ...
    setData((prev) => ({
      ...prev,
      logs: [...prev.logs, { id: uid(), ...logForm, title: logForm.title.trim(), location: logForm.location.trim(), detail: logForm.detail.trim() }],
    }));
  ...
  function saveEditExpense(id) {
    ...
    if (!amount) return alert("金额要大于 0");
    const participants = editExpenseForm.isAA
      ? editExpenseForm.participants.filter((p) => people.includes(p))
      : [];
      ...
      expenses: prev.expenses.map((expense) =>
        expense.id === id
          ? {
              ...
              amount,
              participants: editExpenseForm.isAA ? participants : [],
            }
          : expense
      ),
    }));
  ...
  function exportJson() {
    const filename = `${data.tripName || "旅行账本"}-数据备份.json`;
    downloadText(filename, JSON.stringify(data, null, 2));
  }

  function exportCsv() {
    const header = ["日期", "项目", "金额", "分类", "付款人", "是否AA", "参与人", "我承担", "备注"];
    const rows = sortedExpenses.map((e) => [
      ...
      e.payer,
      e.isAA ? "是" : "否",
      e.isAA ? (e.participants || []).join("/") : "不参与分账",
      shareFor(e, data.me).toFixed(2),
      e.note || "",
    ]);
    const csv = [header, ...rows]
      .map((row) => row.map((cell) => `"${String(cell).replaceAll('"', '""')}"`).join(","))
      .join("\n");
    downloadText(`${data.tripName || "旅行账本"}-消费明细.csv`, "\ufeff" + csv, "text/csv;charset=utf-8");
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-sky-50 to-emerald-50 p-5 text-slate-900">
      <div className="mx-auto max-w-7xl space-y-5">
        <motion.header
          ...
          className="rounded-[2rem] bg-white/80 p-6 shadow-sm ring-1 ring-slate-200"
        >
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <div className="mb-2 inline-flex items-center gap-2 rounded-full bg-slate-100 px-3 py-1 text-sm text-slate-600">
                <WalletCards className="h-4 w-4" /> 旅行账本 · 本地保存 · 可导出纪念图
              </div>
              <input
                className="w-full bg-transparent text-4xl font-bold tracking-tight outline-none lg:text-5xl"
                value={data.tripName}
                onChange={(e) => setData((prev) => ({ ...prev, tripName: e.target.value }))}
              />
              <p className="mt-2 text-slate-600">记录哪一天去了哪里、花了多少钱，以及 AA 项最后谁该补给谁。</p>
            </div>
        ...
        <div className="grid gap-5 lg:grid-cols-[1.05fr_1.95fr]">
          <div className="space-y-5">
              ...
              <div className="space-y-4">
                <Field label="参与人（用顿号/逗号/空格分隔）">
                  <div className="flex gap-2">
                    <input className="w-full rounded-2xl border border-slate-200 bg-white px-3 py-2 outline-none focus:border-slate-400" value={peopleDraft} onChange={(e) => setPeopleDraft(e.target.value)} />
                    <Button variant="soft" onClick={applyPeople}>应用</Button>
                  </div>
                </Field>
                <Field label="我是谁（用于计算“我承担多少”）">
                  <select className="w-full rounded-2xl border border-slate-200 bg-white px-3 py-2 outline-none focus:border-slate-400" value={data.me} onChange={(e) => setData((prev) => ({ ...prev, me: e.target.value }))}>
                    {people.map((p) => <option key={p}>{p}</option>)}
                  </select>
                </Field>
                <div className="rounded-2xl bg-slate-50 p-3 text-sm leading-6 text-slate-600">
                  <b>规则：</b>勾选 AA 后，金额会按参与人平分；不勾选 AA，则视为付款人的个人消费，不进入互相结算。
                </div>
              </div>
            </Card>
            ...
            <Card>
              ...
              <form className="space-y-4" onSubmit={addExpense}>
                ...
                <label className="flex cursor-pointer items-center gap-3 rounded-2xl bg-slate-50 p-3 text-sm font-medium text-slate-700">
                  <input type="checkbox" className="h-4 w-4" checked={expenseForm.isAA} onChange={(e) => setExpenseForm({ ...expenseForm, isAA: e.target.checked, participants: e.target.checked ? people : [] })} />
                  这笔参与 AA / 分账
                </label>
                {expenseForm.isAA && (
                  <div>
                    <div className="mb-2 text-sm font-medium text-slate-600">参与 AA 的人</div>
                    <div className="flex flex-wrap gap-2">
                      {people.map((p) => {
                        const checked = expenseForm.participants.includes(p);
                        return (
                          <label key={p} className={`cursor-pointer rounded-full px-3 py-1.5 text-sm ring-1 ${checked ? "bg-slate-900 text-white ring-slate-900" : "bg-white text-slate-600 ring-slate-200"}`}>
                            <input
                              className="hidden"
                              type="checkbox"
                              checked={checked}
                              onChange={() => setExpenseForm((prev) => ({
                                ...prev,
                                participants: checked ? prev.participants.filter((x) => x !== p) : [...prev.participants, p],
                              }))}
                            />
                            {p}
                          </label>
                    ...
                    <div className="mt-2 text-sm text-slate-500">
                      当前每人承担：¥{money(safeAmount(expenseForm.amount) / Math.max(1, expenseForm.participants.length))}
                    </div>
                  </div>
                )}
          ...
          <div className="space-y-5">
            ...
            <Card>
              <div className="mb-4 flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  {["全部", "AA", "不AA"].map((filter) => (
                    <button
                      key={filter}
                      type="button"
                      className={`rounded-full px-3 py-1.5 text-sm font-medium transition ${expenseFilter === filter ? "bg-slate-900 text-white" : "bg-slate-100 text-slate-600 hover:bg-slate-200"}`}
                      onClick={() => setExpenseFilter(filter)}
                    >
                      {filter}
                    </button>
                  ))}
                  <div className="rounded-full bg-slate-50 px-3 py-1.5 text-sm text-slate-500">
                    当前显示 {filteredExpenses.length} / {sortedExpenses.length} 条
                  </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full min-w-[860px] border-separate border-spacing-y-2 text-left text-sm">
                  <thead className="text-slate-500">
                    <tr>
                      <th className="px-3">日期</th><th className="px-3">项目</th><th className="px-3">金额</th><th className="px-3">分类</th><th className="px-3">付款人</th><th className="px-3">分账</th><th className="px-3">{data.me}承担</th><th className="px-3">操作</th>
                    </tr>
                  </thead>
                  <tbody>
                    {filteredExpenses.length === 0 ? (
                      <tr><td colSpan={8} className="rounded-2xl bg-slate-50 px-3 py-8 text-center text-slate-500">还没有消费记录。可以先录入“竹筏 300，三人 AA”试一下。</td></tr>
                    ) : filteredExpenses.map((e) => {
                      const isEditing = editingExpenseId === e.id;
                      const editingExpense = editExpenseForm
                        ? { ...e, ...editExpenseForm, amount: safeAmount(editExpenseForm.amount) }
                        : e;
                      return (
                        <tr key={e.id} className="bg-slate-50 align-top">
                          <td className="rounded-l-2xl px-3 py-3 text-slate-600">
                            {isEditing ? (
                              <input
                                type="date"
                                className="w-36 rounded-xl border border-slate-200 bg-white px-2 py-1 outline-none focus:border-slate-400"
                                value={editExpenseForm.date}
                                onChange={(event) => setEditExpenseForm((prev) => ({ ...prev, date: event.target.value }))}
                              />
                            ) : e.date}
                          </td>
                          <td className="px-3 py-3">
                            {isEditing ? (
                              <div className="space-y-2">
                                <input
                                  className="w-40 rounded-xl border border-slate-200 bg-white px-2 py-1 outline-none focus:border-slate-400"
                                  value={editExpenseForm.title}
                                  onChange={(event) => setEditExpenseForm((prev) => ({ ...prev, title: event.target.value }))}
                                />
                                <input
                                  className="w-40 rounded-xl border border-slate-200 bg-white px-2 py-1 text-xs outline-none focus:border-slate-400"
                                  placeholder="备注"
                                  value={editExpenseForm.note}
                                  onChange={(event) => setEditExpenseForm((prev) => ({ ...prev, note: event.target.value }))}
                                />
                              </div>
                            ) : (
                              <>
                          ...
                          <td className="px-3 py-3 font-semibold">
                            {isEditing ? (
                              <input
                                type="text"
                                inputMode="decimal"
                                className="w-24 rounded-xl border border-slate-200 bg-white px-2 py-1 outline-none focus:border-slate-400"
                          ...
                          <td className="px-3 py-3 text-slate-600">
                            {isEditing ? (
                              <select
                                className="w-28 rounded-xl border border-slate-200 bg-white px-2 py-1 outline-none focus:border-slate-400"
                          ...
                          <td className="px-3 py-3 text-slate-600">
                            {isEditing ? (
                              <select
                                className="w-24 rounded-xl border border-slate-200 bg-white px-2 py-1 outline-none focus:border-slate-400"
                                value={editExpenseForm.payer}
                                onChange={(event) => setEditExpenseForm((prev) => ({ ...prev, payer: event.target.value }))}
                              >
                            ...
                            {isEditing ? (
                              <div className="space-y-2">
                                <label className="inline-flex items-center gap-2 text-xs text-slate-600">
                                  <input
                                    type="checkbox"
                                    checked={editExpenseForm.isAA}
                                    onChange={(event) => setEditExpenseForm((prev) => ({
                                      ...prev,
                                      isAA: event.target.checked,
                                      participants: event.target.checked ? (prev.participants.length ? prev.participants : people) : [],
                                    }))}
                                  />
                                  AA
                                </label>
                                {editExpenseForm.isAA && (
                                  <div className="flex flex-wrap gap-1">
                                    {people.map((person) => {
                                      const checked = editExpenseForm.participants.includes(person);
                                      return (
                                        <button
                                          key={person}
                                          type="button"
                                          className={`rounded-full px-2 py-1 text-xs ${checked ? "bg-slate-900 text-white" : "bg-slate-200 text-slate-600"}`}
                                          onClick={() => setEditExpenseForm((prev) => ({
                                            ...prev,
                                            participants: checked
                                              ? prev.participants.filter((x) => x !== person)
                              ...
                              e.isAA ? <span className="rounded-full bg-emerald-100 px-2 py-1 text-xs font-medium text-emerald-700">AA：{(e.participants || []).join("、")}</span> : <span className="rounded-full bg-slate-200 px-2 py-1 text-xs font-medium text-slate-600">不 AA</span>
                            )}
                          </td>
                          <td className="px-3 py-3 font-semibold">¥{money(isEditing ? shareFor(editingExpense, data.me) : shareFor(e, data.me))}</td>
                          <td className="rounded-r-2xl px-3 py-3">
                            {isEditing ? (
                              <div className="flex gap-1">
                                <Button variant="soft" onClick={() => saveEditExpense(e.id)}><Save className="h-4 w-4" /></Button>
                                <Button variant="ghost" onClick={cancelEditExpense}><X className="h-4 w-4" /></Button>
                              </div>
                            ) : (
                              <div className="flex gap-1">
                                <Button variant="ghost" onClick={() => startEditExpense(e)}><Pencil className="h-4 w-4" /></Button>
                                <Button variant="ghost" onClick={() => removeExpense(e.id)}><Trash2 className="h-4 w-4" /></Button>
                              </div>
                            )}
                          </td>
                        </tr>
            ...
            <div className="grid gap-5 lg:grid-cols-2">
              <Card>
                <div className="mb-4 text-lg font-semibold">AA 结算结果</div>
                {stats.transfers.length === 0 ? (
                  <div className="rounded-2xl bg-slate-50 p-4 text-sm text-slate-500">目前没有需要互相转账的 AA 款项。</div>
                ) : (
                  <div className="space-y-3">
                    {stats.transfers.map((t, idx) => (
                      <div key={idx} className="flex items-center justify-between rounded-2xl bg-slate-50 p-3">
                        <div className="font-medium"><span className="text-rose-600">{t.from}</span> 转给 <span className="text-emerald-700">{t.to}</span></div>
                        <div className="text-lg font-bold">¥{money(t.amount)}</div>
                      </div>
                    ))}
                  </div>
                )}
                <div className="mt-4 grid gap-2 text-sm">
                  {people.map((p) => (
                    <div key={p} className="flex justify-between rounded-xl bg-white px-2 py-1 text-slate-600">
                      <span>{p}：AA垫付 ¥{money(stats.paidAA[p])} / AA应承担 ¥{money(stats.shareAA[p])}</span>
                      <span className="font-medium text-slate-800">净额 {stats.nets.find((x) => x.name === p)?.net >= 0 ? "+" : ""}{money(stats.nets.find((x) => x.name === p)?.net || 0)}</span>
                    </div>
                  ))}
                </div>
              </Card>
              ...
              <Card>
                <div className="mb-4 text-lg font-semibold">行程记录</div>
                {sortedLogs.length === 0 ? (
                  <div className="rounded-2xl bg-slate-50 p-4 text-sm text-slate-500">还没有行程记录。可以写“到桂林北”“坐竹筏”“逛西街”等。</div>
                ) : (
                  <div className="space-y-3">
                    {sortedLogs.map((log) => (
                      <div key={log.id} className="rounded-2xl bg-slate-50 p-3">
                        <div className="flex items-start justify-between gap-3">
                          <div>
                            ...
                            <div className="mt-1 font-semibold">{log.title}</div>
                            {log.detail && <div className="mt-1 text-sm leading-6 text-slate-600">{log.detail}</div>}
                          </div>
                          <Button variant="ghost" onClick={() => removeLog(log.id)}><Trash2 className="h-4 w-4" /></Button>
                        </div>
                      </div>
                    ))}
                  </div>
            ...
            <Card>
              <div className="mb-4 text-lg font-semibold">统计图</div>
              <div className="grid gap-5 lg:grid-cols-2">
                <div>
                  <div className="mb-3 text-sm font-medium text-slate-600">每天消费</div>
                  <div className="space-y-3">
                    {stats.daily.length === 0 ? <div className="text-sm text-slate-500">暂无数据</div> : stats.daily.map((d) => (
                      <div key={d.date}>
                        <div className="mb-1 flex justify-between text-sm"><span>{d.date}</span><span>总 ¥{money(d.total)} / 我 ¥{money(d.myShare)}</span></div>
                        <div className="h-3 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full bg-slate-800" style={{ width: `${Math.max(4, (d.total / maxDaily) * 100)}%` }} /></div>
                      </div>
                    ))}
                  </div>
                </div>
                <div>
                  ...
                  <div className="space-y-3">
                    {stats.categories.length === 0 ? <div className="text-sm text-slate-500">暂无数据</div> : stats.categories.map((c) => (
                      <div key={c.category}>
                        <div className="mb-1 flex justify-between text-sm"><span>{c.category}</span><span>总 ¥{money(c.total)} / 我 ¥{money(c.myShare)}</span></div>
                        <div className="h-3 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full bg-slate-800" style={{ width: `${Math.max(4, (c.total / maxCategory) * 100)}%` }} /></div>
                      </div>
                    ))}
                  </div>
                </div>
            ...
            <Card className="overflow-hidden">
              <div className="mb-4 flex items-center justify-between gap-3">
                <div className="text-lg font-semibold">纪念账单预览</div>
                <Button onClick={exportImage}><Camera className="h-4 w-4" />导出这张图</Button>
              </div>
              <div ref={reportRef} className="rounded-[2rem] bg-slate-50 p-8">
                <div className="rounded-[2rem] bg-white p-7 shadow-sm ring-1 ring-slate-200">
                  <div className="mb-6 flex flex-col gap-2 border-b border-slate-100 pb-5 md:flex-row md:items-end md:justify-between">
                    <div>
                      <div className="text-sm font-medium text-slate-500">旅行回顾账单</div>
                      <div className="mt-1 text-3xl font-bold">{data.tripName || "旅行账本"}</div>
                      <div className="mt-2 text-sm text-slate-500">参与人：{people.join("、")}</div>
                    </div>
                    <div className="text-right text-sm text-slate-500">生成于 {new Date().toLocaleDateString("zh-CN")}</div>
                  </div>

                  <div className="grid gap-3 md:grid-cols-4">
                    <div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs text-slate-500">总消费</div><div className="mt-1 text-2xl font-bold">¥{money(stats.total)}</div></div>
                    <div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs text-slate-500">AA 总额</div><div className="mt-1 text-2xl font-bold">¥{money(stats.aaTotal)}</div></div>
                    <div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs text-slate-500">非 AA</div><div className="mt-1 text-2xl font-bold">¥{money(stats.personalTotal)}</div></div>
                    <div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs text-slate-500">{data.me}承担</div><div className="mt-1 text-2xl font-bold">¥{money(stats.myShare)}</div></div>
                  </div>

                  <div className="mt-6 grid gap-6 md:grid-cols-2">
                    <div>
                      <div className="mb-3 text-base font-semibold">每天做了什么</div>
                      <div className="space-y-3">
                        {sortedLogs.length === 0 ? <div className="rounded-2xl bg-slate-50 p-4 text-sm text-slate-500">还没有行程记录</div> : sortedLogs.slice(0, 8).map((log) => (
                          <div key={log.id} className="rounded-2xl bg-slate-50 p-3">
                            <div className="text-xs text-slate-500">{log.date}{log.location ? ` · ${log.location}` : ""}</div>
                            <div className="mt-1 font-medium">{log.title}</div>
                            {log.detail && <div className="mt-1 line-clamp-2 text-sm text-slate-600">{log.detail}</div>}
                          </div>
                        ))}
                      </div>
                    </div>
                    <div>
                      <div className="mb-3 text-base font-semibold">AA 转账</div>
                      <div className="space-y-3">
                        {stats.transfers.length === 0 ? <div className="rounded-2xl bg-slate-50 p-4 text-sm text-slate-500">暂无需要转账的 AA 款项</div> : stats.transfers.map((t, idx) => (
                          <div key={idx} className="flex items-center justify-between rounded-2xl bg-slate-50 p-3">
                            <span>{t.from} → {t.to}</span><b>¥{money(t.amount)}</b>
                          </div>
                        ))}
                      </div>

                      <div className="mt-6 mb-3 text-base font-semibold">分类概览</div>
                      <div className="space-y-2">
                        {stats.categories.slice(0, 6).map((c) => (
                          <div key={c.category} className="flex justify-between rounded-xl bg-slate-50 px-3 py-2 text-sm"><span>{c.category}</span><span>¥{money(c.total)}</span></div>
                        ))}
                      </div>
                    </div>
                  </div>
